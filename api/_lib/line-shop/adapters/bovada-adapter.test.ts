/**
 * bovada-adapter.test.ts
 *
 * Vitest suite for BovadaAdapter (Phase 12-01).
 *
 ...
 *  2. isEnabled() returns false when BOVADA_* creds are absent (disabled by default)
 ...
 *  7. assertNoOrderSurface(new BovadaAdapter()) passes (no forbidden methods)
 *
 ...
 *
 * BOOK-07, D-02/D-03/D-04/D-05/D-06, T-12-01..T-12-05
 */
...
describe('BovadaAdapter', () => {
  let adapter: BovadaAdapter

  beforeEach(() => {
    adapter = new BovadaAdapter()
    vi.clearAllMocks()
  ...
  // ── (1) isEnabled() — VERCEL guard (D-02, T-12-01) ──────────────────────────

  describe('isEnabled() — VERCEL guard (D-02, T-12-01)', () => {
  ...
  describe('isEnabled() — creds gate, disabled by default (D-03, BOOK-07)', () => {
    it('returns false when BOVADA_* creds are absent (disabled by default — no account yet)', () => {
      delete process.env.VERCEL
      delete process.env.BOVADA_USERNAME
      delete process.env.BOVADA_PASSWORD
    ...
    it('returns false when only BOVADA_USERNAME is set (missing password)', () => {
      delete process.env.VERCEL
      process.env.BOVADA_USERNAME = 'myuser'
      delete process.env.BOVADA_PASSWORD

      expect(adapter.isEnabled()).toBe(false)
    })

    it('returns false when only BOVADA_PASSWORD is set (missing username)', () => {
      delete process.env.VERCEL
      delete process.env.BOVADA_USERNAME
      process.env.BOVADA_PASSWORD = 'mypass'
  ...
  describe('fetchMarket() — disabled adapter (D-02/D-03)', () => {
    it('returns null (no throw) when isEnabled() is false due to VERCEL', async () => {
      process.env.VERCEL = '1'
      process.env.BOVADA_USERNAME = 'u'
      process.env.BOVADA_PASSWORD = 'p'
  ...
  describe('fetchMarket() — stale session redirect (D-04, T-12-03, Pitfall 1)', () => {
    it('returns null and upserts scrape_health session_expired on /login redirect', async () => {
      ...
      vi.mocked(mockPage.url).mockReturnValue('https://www.bovada.lv/login')
    ...
    it('detects /signin path as a login redirect', async () => {
      ...
      vi.mocked(existsSync).mockReturnValue(true)
      vi.mocked(mockPage.url).mockReturnValue('https://www.bovada.lv/signin')
  ...
  describe('fetchMarket() — DOM drift (T-12-03, Pitfall 2)', () => {
    it('returns null and upserts scrape_health dom_drift when odds selector times out', async () => {
      ...
      vi.mocked(existsSync).mockReturnValue(true)
      vi.mocked(mockPage.url).mockReturnValue('https://www.bovada.lv/sports/baseball')
  ...
  describe('fetchMarket() — successful scrape (D-06, BOOK-07)', () => {
    ...
    it('snapshots carry RAW impliedProb (sum > 1.0 for a vigged market)', async () => {
      ...
      const market2 = { ...CANONICAL_MARKET, id: 'market-uuid-bovada-02' }
  ...
  describe('fetchMarket() — fail-closed on errors (T-12-03, D-06)', () => {
  ...
  describe('read-only enforcement (BOOK-07, D-05, T-12-01)', () => {
    it('assertNoOrderSurface(new BovadaAdapter()) passes — no forbidden methods', () => {
      expect(() => assertNoOrderSurface(new BovadaAdapter())).not.toThrow()
    })

    it('BovadaAdapter prototype has no forbidden method names', () => {
      const forbidden = [
        'placeBet',
        'confirmBet',
        'submitOrder',
      ...
      for (const name of forbidden) {
        expect(names, `forbidden method '${name}' must not exist on BovadaAdapter`).not.toContain(name)
      }
    })
  })
  ...
  describe('adapter metadata', () => {
    it('has name="bovada" and sourceConfidence="scraped"', () => {
      expect(adapter.name).toBe('bovada')
      expect(adapter.sourceConfidence).toBe('scraped')
    })

    it('is its own class — NOT an instance of DgsPphAdapter', async () => {
      // Dynamic import to avoid circular module issues; just check constructor name
      expect(adapter.constructor.name).toBe('BovadaAdapter')
    })
  })
})
