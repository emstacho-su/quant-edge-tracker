/**
 * Kalshi taker-fee math — single source of truth (client copy).
 * Mirror of api/_lib/line-shop/kalshi-fee.ts — keep in sync.
 *
 ...
 *
 *     contracts        = floor(stake / P)
 *     fee_per_contract = ceil(mult × P × (1 − P) × 100) cents
 *     total_fee_$      = contracts × fee_per_contract / 100
 *
 * Use this for display ("you will pay $X in fees on this Kalshi leg"),
 * NOT for the price-comparison or stake-split math — use
 * kalshiEffectiveImpliedProb / kalshiEffectiveDecimalOdds for that
 * (un-rounded form, per D-13).
 *
 * Returns `{ contracts: 0, … }` and total fee 0 for invalid inputs.
 *
 * @param stake       - Stake amount in dollars
 * @param impliedProb - Raw implied probability P = 1/decimal
 * @param side        - 'taker' (default) or 'maker'
 */
